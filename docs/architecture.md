...
FOCTwin 0.5.0b3 разделяет управление мотором, выполнение локальных программ, хранение данных и
...
Команды остаются буквальными. Компилятор лишь отслеживает известную прямую последовательность
`AR-12345`, `AT0`, `AC0`, `ALU...`, `A...` и отклоняет цель `Uq`, превышающую видимый `ALU`; он
ничего не дописывает и не меняет в исходнике.
...
Путь последнего проекта и его родительского каталога хранится в пользовательских настройках.
Существующий проект автоматически открывается после перезапуска; отсутствующий путь не создаётся.
